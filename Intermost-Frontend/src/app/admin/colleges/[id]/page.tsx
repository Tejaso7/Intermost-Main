...
function collegeToForm(college: College): CollegeForm {
  return {
    ...
    tuition_per_year: college.fees?.tuition_fee_per_year || '',
    hostel_per_year: college.fees?.hostel_fee_per_year || '',
    total_course_fee: college.fees?.total_package || '',
    recognition: college.recognition?.map(r => r.name) || ['NMC', 'WHO'],
    eligibility_marks: college.eligibility?.minimum_percentage || '',
...
function formToCollege(form: CollegeForm): Partial<College> {
  return {
    ...
    rankings: {
      world_rank: form.world_rank || undefined,
    },
    fees: {
      tuition_fee_per_year: form.tuition_per_year,
      hostel_fee_per_year: form.hostel_per_year,
      total_package: form.total_course_fee,
    },
    eligibility: {
      minimum_percentage: form.eligibility_marks,
    },
    recognition: form.recognition.map(name => ({ name, verified: true })),
    facilities: form.facilities.map(name => ({ icon: 'check', name, description: '' })),
    banner_image: form.banner_image,
    logo: form.logo,
    meta: {
      is_featured: form.is_featured,
      is_active: form.is_active,
      is_nmc_approved: form.recognition.includes('NMC'),
      is_who_listed: form.recognition.includes('WHO'),
      display_order: 0,
